 ...
 */
export async function enhanceAnalysisWithAI(baseAnalysis) {
        ...
        for (const flag of parsed.additionalRedFlags) {
          if (flag && flag.issue && !existingIds.has(flag.id)) {
            baseAnalysis.redFlags.push({
              ...
              riskLevel: ['critical', 'warning', 'advisory'].includes(flag.riskLevel) ? flag.riskLevel : 'warning',
 ...
 */
export async function answerQuestionWithAI(question, rawText) {
  const ai = getGeminiClient();
  if (!ai) return null;
