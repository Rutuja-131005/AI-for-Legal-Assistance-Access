...
import { apiLimiter, aiOperationLimiter, uploadLimiter } from './server/middleware/rateLimiter';
import {
  validateRequestBody,
  parseDocumentSchema,
...
app.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const currentAnalysis: ContractAnalysis = analysis || analyzeDocumentText(textToUse);
